...
Mode nine is the dip scanner, its own tab. This is the pattern behind an established, well known stock falling hard from a recent high, the ZTS style setup: is a big name being cheap actually an opportunity, or is the market simply right. It searches for recognizable companies down roughly 15 percent or more from a recent high, states the real, verified reason for the drop, and reads whether that reaction looks like an overreaction to something fixable or a structural change that justifies the lower price, honestly marking it unclear when the evidence is mixed. Different in kind from the diamond scanner, which only covers sub $5 stocks with a dated catalyst; this one is any size, any price, for the big names.
